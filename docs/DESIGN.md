# Diseño: cómo se llegó hasta aquí

Este documento es el **diario de desarrollo** del proyecto: las decisiones de diseño de cada fase,
por qué se tomaron, y los experimentos que salieron mal. Está ordenado cronológicamente porque el
razonamiento de una fase solo se entiende sabiendo lo que había antes.

**No hace falta leerlo para usar el proyecto.** Para eso está el [README](../README.md); para
escribir el cliente de Unity, [PROTOCOL.md](PROTOCOL.md). Esto es para quien quiera saber por qué
las cosas son como son —o para quien tenga que seguir tocándolas—.

| Fase | Qué añadió |
|---|---|
| 2 | El mapa como grafo, con validación y carga desde JSON |
| 3 | A\* con penalizaciones, el `Agent` y la `Simulation` |
| 5 | Detección de conflictos, la política baseline y el deadlock |
| 6 | El entorno de Q-Learning: estado, acciones y recompensa |
| 7 | El bucle de entrenamiento y los dos modos (`train` / `evaluate`) |
| 8 | A\* y Q-Learning juntos en el tick, y el desatasco del motor |
| 9 | Métricas pareadas: medir en vez de suponer |
| 10 | Cinco escenarios reproducibles, y dónde el Q-Learning sí sirve |
...
recargar Unity. Los detalles del comando están en [PROTOCOL.md](PROTOCOL.md#2-comandos).
